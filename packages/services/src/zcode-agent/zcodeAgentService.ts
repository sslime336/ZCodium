...
> {
  ...
  /** Dynamic workflow gate resolver; absent means disabled. */
...
export function createZCodeAgentService(
    ...
    const disposables = [
      ...
      client.onRequest((request) => {
        ...
        if (request.method === zcodeProtocolMethods.interactionRequestProviderRuntimeHeaders) {
          ...
          // Official account request-auth resolution was removed (harness-simplification layer C/D);
          // without a resolver an unattended request would hang until the CLI timeout, so fail fast.
  ...
  async function getOrStartReadOnlyClient(
    ...
    // Dynamic workflow policy is a workspace-level fact synced to the CLI before any session runs,
    // so v4 cold-restore also sees the tool surface.
    ...
    const dynamicWorkflowPolicyReady = (async () => {
      ...
      try {
        ...
        // -32601 is a normal degrade on older CLIs; other errors only warn, never block readiness.
   ...
   * Remote workspaces are also supported, so this gate ignores workspaceIdentity / remoteSessionId.
   */
