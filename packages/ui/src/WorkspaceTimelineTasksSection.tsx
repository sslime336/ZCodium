/* eslint-disable max-lines -- timeline 承载本地 scoped 查询与任务操作分发，先集中保持链路清晰。 */
...
export function WorkspaceTimelineTasksSection({
  ...
  const hasKnownMore = localHasMore || total > items.length;
  const currentLimitFilled = sortedItems.length >= visibleTaskLimit;
  // hasMore 偶尔会在下一轮请求完成前保持旧值。
  ...
  useEffect(() => {
    ...
    logger.debug("[WorkspaceTimelineTasksSection] scopedWorkspaceTabs", {
      workspaceCount: scopedWorkspaceTabs.length,
    });
  }, [scopedWorkspaceTabs]);
